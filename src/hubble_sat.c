...
{
	uint64_t synced_interval_s = (hubble_internal_utc_time_get() -
				      hubble_internal_utc_time_synced()) /
				     1000;
	return HUBBLE_MIN(
		UINT8_MAX,
		(uint32_t)(((double)CONFIG_HUBBLE_SAT_NETWORK_DEVICE_TDR / 1000000) *
			   ((double)synced_interval_s / interval_s)));
}
