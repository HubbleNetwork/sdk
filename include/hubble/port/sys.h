 ...
 * objects that are platform dependend, like struct hubble_timer.
 */
 ...
 * @param expiration_ms The expiration interval in milliseconds. This value
 *                       determines both the initial delay before the first
 *                       expiration and the period for subsequent expirations.
 *
 * @return Returns 0 indicating success. Non-zero value otherwise.
 */
int hubble_timer_start(struct hubble_timer *timer, int64_t expiration_ms);
